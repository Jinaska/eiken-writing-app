...
module.exports = async function handler(req, res) {
  ...
  // 採点の寛容さ（やや甘めに、学習者を励ます方針）
  const leniencyRule =
    `# 採点方針（やや甘め）\n` +
    `学習者を励ます方針で、全体的にやや甘めに採点してください。各観点で迷ったら高い方の点を選んでください。\n` +
    `- 語彙: 高度・難解な語彙は必須ではありません。そのレベルとして意味が正確に伝わる適切な語彙が使えていれば3〜4点を与えてよく、` +
    `平易な語彙でも大きな誤りがなく内容がきちんと伝わっていれば過度に減点しないでください。\n` +
    `- 文法: コミュニケーションを妨げない軽微なミスは大きく減点しないでください。`;

  // 要約の採点ルール（英検 英文要約の観点）
  const summaryRule =
    `これは英検の「英文要約（English Summary）」の採点です。受験者は本文(ARTICLE)を読み、` +
    `${wordTarget}に要約します。次の方針で4観点（内容/構成/語彙/文法）各0〜4点・計16点で採点してください。\n` +
    `- 内容: 本文の主要なポイント（賛成側・反対側などの要点）を過不足なく正確に捉えているか。本文に無い情報や自分の意見・例を加えていないか（加えていれば減点）。\n` +
    `- 構成: 要点が論理的につながり、要約として一つのまとまりになっているか。\n` +
    `- 語彙: 語彙が適切で意味が正確に伝わるか。完全な言い換え(paraphrase)は必須ではありません。本文の語句を多少そのまま使っていても、主要点が正しく伝わっていれば語彙点を過度に下げないでください（少しでも自分の言葉でまとめられていれば十分です）。\n` +
    `- 文法: 文法・綴りがおおむね正確か。軽微なミスは大きく減点しない。\n` +
    `重要: 本文の主旨から外れている場合や、本文の数文をほぼ丸ごとコピーしただけの場合は内容点を下げてください（ただし多少の流用は許容）。`;
